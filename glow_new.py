...
class Glow(nn.Module):
    def __init__(
        ...
        self.z_shapes, self.cum_idxs = calc_z_shapes(3,64, n_flow, n_block)
    ...
    def get_loss(self, data_batch, num_bits=0):
        ...
        npixel = np.prod(3*64*64)
        if num_bits == 0:
            log_p,logdet,_ = self(data_batch.to(self.device))
            loss = log_p + logdet
        else:
            num_bins = 2 ** num_bits
            dbatch = data_batch + torch.rand_like(data_batch) / num_bins
            log_p,logdet,_ = self(dbatch.to("cuda"))
    ...
    def sample(self, num_samples, temp=1.):
        latent_dist = dists.normal.Normal(loc=torch.tensor(0, dtype=torch.float32).to("cuda"), scale=torch.tensor(1, dtype=torch.float32).to("cuda"))
        z = latent_dist.sample([num_samples, np.prod(3*64*64)]) * temp
